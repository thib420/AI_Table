...
export class RoutePreloader {
  ...
  private moduleManifest: Record<string, ModuleManifest> = {
    // 'mailbox': {
    //   component: () => import('@/features/mailbox').then(m => m.MailboxPage),
    //   dependencies: ['@/features/mailbox/hooks/useUnifiedMailbox'],
    //   dataRequirements: ['emails', 'folders'],
    //   criticalData: ['folders'] // Load folders first for mailbox structure
    // },
    // 'crm': {
    //   component: () => import('@/features/crm').then(m => m.CRMPage),
    //   dependencies: ['@/features/crm/services/UnifiedCRMService'],
    //   dataRequirements: ['contacts', 'meetings', 'emails'],
    //   criticalData: ['contacts'] // Load contacts first for CRM
    // },
    // 'campaigns': {
    //   component: () => import('@/features/campaigns').then(m => m.EmailCampaignPage),
    //   dependencies: ['@/features/campaigns/components/EmailSequenceBuilder'],
